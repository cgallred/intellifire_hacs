...
async def _async_validate_host_input(host: str, dhcp_mode: bool = False) -> str:
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def _async_find_fireplaces(self):
    ...
    async def _async_validate_api_access_and_create_or_update(
        self,
        *,
        host: str,
        username: str,
        ...
        if existing_entry:
            LOGGER.error("---Updating existing entry 1")
            self.hass.config_entries.async_update_entry(
                existing_entry, data=data, options=options
            )
            await self.hass.config_entries.async_reload(existing_entry.entry_id)
            return self.async_abort(reason="reauth_successful")
        LOGGER.error("---Creating new Entry 2")
        return self.async_create_entry(
            title=f"Fireplace {serial}", data=data, options=options
        )
        ...
        control_schema = vol.Schema(
            {
                ...
                vol.Optional(CONF_CLOUD_READ_MODE, default=False): cv.boolean,
                vol.Optional(CONF_CLOUD_CONTROL_MODE, default=False): cv.boolean,
            }
        )
            ...
            control_schema = vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_CLOUD_READ_MODE,
                        default=user_input.get(CONF_CLOUD_READ_MODE, False),
                    ): cv.boolean,
                    vol.Optional(
                        CONF_CLOUD_CONTROL_MODE,
                        default=user_input.get(CONF_CLOUD_CONTROL_MODE, False),
                    ): cv.boolean,
                }
            )

            try:
                return await self._async_validate_api_access_and_create_or_update(
                    host=self._host,
                    username=user_input[CONF_USERNAME],
                    password=user_input[CONF_PASSWORD],
                    serial=self._serial,
                    cloud_control=user_input.get(CONF_CLOUD_CONTROL_MODE, False),
                    cloud_read=user_input.get(CONF_CLOUD_READ_MODE, False),
                )
    ...
    async def _async_validate_ip_and_continue(self, host: str) -> FlowResult:
        ...
        self._serial = await _async_validate_host_input(host)
    ...
    async def async_step_user(
        ...
        await self._async_find_fireplaces()
    ...
    async def async_step_dhcp(self, discovery_info: DhcpServiceInfo) -> FlowResult:
        ...
        self._async_abort_entries_match({CONF_HOST: host})
        try:
            self._serial = await _async_validate_host_input(host, dhcp_mode=True)
        except (ConnectionError, ClientConnectionError):
            LOGGER.debug(
    ...
    async def async_step_dhcp_confirm(self, user_input=None) -> FlowResult:
        ...
        return self.async_create_entry(
            ...
            options={
                CONF_CLOUD_READ_MODE: False,
                CONF_CLOUD_CONTROL_MODE: False,
            },
        )
...
class IntelliFireOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_options(
        ...
        data_schema = vol.Schema(
            {
                vol.Optional(
                    CONF_CLOUD_READ_MODE,
                    default=self._config_entry.options.get(CONF_CLOUD_READ_MODE, False),
                ): cv.boolean,
                vol.Optional(
                    CONF_CLOUD_CONTROL_MODE,
        ...
        return self.async_show_form(
            ...
            #             vol.Optional(
            #                 CONF_CLOUD_CONTROL_MODE,
            #                 default=self._config_entry.options.get(CONF_CLOUD_CONTROL_MODE, False),
            #             ): cv.boolean,
            #         }
            #     ),
            errors=self._errors,
